...
public class Rat extends SpriteObject {
    ...
    public Rat(byte type){
        ...
        else if (type == 1) door = new Door(3);
        attack = new Attack();
        bed = new Bed();
        peek = new Peek();
    ...
    public boolean execute(SoundHandler soundHandler, Player player, Cat cat, Room room, boolean twitching){
        ...
        twitching = input(soundHandler, player, room, flashlight.getX(), flashlight.getY(), twitching);
        twitching = update(soundHandler, cat, player, room, flashlight, twitching);
        return twitching;
    }

    private boolean input(SoundHandler soundHandler, Player player, Room room, float mx, float my, boolean twitching){
        boolean hovered = hitbox.isHovered(mx, my);
        boolean imageHovered = this.mouseOverWithPanning(mx, my);
        switch (state){
            case 0:
                if (door.input(soundHandler, room, hovered)) hitbox.setCoord(0, 0);
                break;
            case 1:
    ...
    private boolean update(SoundHandler soundHandler, Cat cat, Player player, Room room, Flashlight flashlight, boolean twitching){
        ...
        switch (state){
            case 0:
                ...
                if (catInRoom) {
                    door.pause();
                } else if (cat != null && cat.getState() == 3 && ((
                        side == 0 && cat.getSide() == 2)
                        || (side == 2 && cat.getSide() == 0))){
                    door.slowDown();
                }
                if (door.isSignal()) {
                    boolean cond = cat != null && cat.getState() == 4;
                    if (door.getSameSide() == 3) {
                        ...
                        if (!cond) side = (byte) random.nextInt(3);
                        else {
                            int rand = random.nextInt(2);
            ...
            case 1:
                if (((attack.getKillTimer() == 0 && attack.getReactionTimer() == 0)
                    ...
                    else if (type == 1) Jumpscare.set("game/Shadow Rat/Jumpscare", 0.75f);
    ...
    public void transitionRoomState(byte state){
        ...
        } else if (state == 3) peek.reset(3 - (1.25f * Math.min(type, 1)), 2.25f + (0.25f * type), 1.25f);
