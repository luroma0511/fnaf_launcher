    ...
    public void update(Window window, InputManager inputManager, Room room, Rat rat, Cat cat){
        ...
        if (!attack || GameData.freeScroll) {
            if (room.getFrame() == 0 && room.getState() != 2 && !freeze) {
                x += lookMechanic(x, inputManager.getX(), (float) window.width() / 3, 1, room.getBoundsX(), shakeLimit);
                y += lookMechanic(y, inputManager.getY(), (float) window.height() / 3, 4, room.getBoundsY(), 0);
            }
            snapCooldown = 0.75f;
            ...
            float limit = Time.convertValue(1800);
        ...
        if (snapCooldown == 0) {
            boolean a = rat != null && ((rat.getState() == 0 && rat.getDoor().getCooldown() == 0
                    && cat != null && rat.getState() != cat.getSide())
                    || (rat.getState() == 1 && rat.getSide() != side));
            boolean b = cat != null && cat.getState() == 1 && cat.getSide() != side;
            snapCooldown = 0.5f;
            if (inputManager.getX() < (float) window.width() * 0.2f && side != 0 && (a || b)) side--;
            else if (inputManager.getX() > (float) window.width() * 0.8f && side != 2 && (a || b)) side++;
            else snapCooldown = 0;
