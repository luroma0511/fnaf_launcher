...
public class Game {
    ...
    public Game(){
        ...
        nightTimeLength = 60 * nightMinuteLength;
    ...
    public void update(Engine engine){
        ...
        if (!gameOver) {
            ...
            if (GameData.night == 0) data.mainCastStar = writeStars(data.mainCastStar);
            else if (GameData.night == 1) data.shadowCastStar = writeStars(data.shadowCastStar);
            else data.hellCastStar = writeStars(data.hellCastStar);

            if (GameData.freeScroll){
                if (GameData.night == 0) data.mainCastStarRainbow = writeStars(data.mainCastStarRainbow);
                else if (GameData.night == 1) data.shadowCastStarRainbow = writeStars(data.shadowCastStarRainbow);
                else data.hellCastStarRainbow = writeStars(data.hellCastStarRainbow);
            }
            ...
            if (GameData.hitboxMultiplier < 1 && GameData.hardCassette) {
    ...
    private void renderGame(Engine engine){
        ...
        if (player.isJumpscare() && player.getBlacknessTimes() == 0){
            ...
            if (!Jumpscare.render(engine.appHandler.soundHandler, batch)) {
                gameOver = true;
                whiteAlpha = 1;
                gameOverAlpha = 1;
        ...
        fontManager.setPosition(CameraManager.getX() + 20, CameraManager.getY() + window.height() - fontManager.getLayout().height + 8);
        fontManager.render(batch);
        fontManager.setOutline(0.5f);
        ...
        fontManager.setPosition(CameraManager.getX() + window.width() - fontManager.getLayout().width - 20,
                CameraManager.getY() + window.height() - fontManager.getLayout().height - 40);
        fontManager.render(batch);
        fontManager.setOutline(0.5f);
        fontManager.setColor(0, 0, 0, 1);
    }
    ...
    private int writeStars(int modeStars){
        if (GameData.hitboxMultiplier < 1) {
            if (GameData.hardCassette) modeStars = 5;
            else if (modeStars == 3) modeStars = 4;
            else if (modeStars < 4) modeStars = 2;
        } else if (GameData.hardCassette) {
            if (modeStars == 2) modeStars = 4;
            else if (modeStars < 4) modeStars = 3;
        } else if (modeStars == 0) modeStars = 1;
        return modeStars;
    }
