...
public class BedSide {
    ...
    public boolean input(Hitbox hitbox, Room room, float mx, float my){
        boolean sound = true;
        if ((room.getState() != 0 || room.getFrame() != 0 || !hitbox.isHovered(mx, my)) && !retreat){
            flashTime = Time.decreaseTimeValue(flashTime, 0, 2);
            sound = false;
        } else flashTime = Time.increaseTimeValue(flashTime, 1.25f, 1);
        return sound;
    }
