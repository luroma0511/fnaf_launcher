...
public class Room extends SpriteObject {
    ...
    Music tapeMusic = Gdx.audio.newMusic(Gdx.files.absolute(PathConstant.getAssetsPath() + "sounds/tapemusic.wav"));
    ...
    public void render(SpriteBatch batch, Characters characters, Room room, Flashlight flashlight){
        ...
        if ((int) frame == 0) {
            if (state != 2){
                ...
                float multiplier = 1;
                if (Challenges.laserPointer) multiplier = 1.75f;
        ...
        if (state != 2) batch.draw(region, 0, 0);
            ...
            if ((int) frame == 0) {
                ...
                batch.draw(region, CameraManager.getX() + 520, CameraManager.getY() + 157.5f, 492.5f, 258.75f);
            }
        }
