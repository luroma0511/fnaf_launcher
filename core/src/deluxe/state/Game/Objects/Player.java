...
public class Player {
    ...
    public void update(Room room){
        if (room.getFrame() == 0) {
            x += lookMechanic(x, Candys3Deluxe.inputManager.getX(), (float) Candys3Deluxe.width / 3, 1, room.getBoundsX());
            y += lookMechanic(y, Candys3Deluxe.inputManager.getY(), (float) Candys3Deluxe.height / 3, 2, room.getBoundsY());
        }
        ...
        if (room.getFrame() == 0 && y == 0) buttonFade = Time.increaseTimeValue(buttonFade, 1, 4);
...
//        purpleSpeed = engine.increaseTimeValue(purpleSpeed, 1, 0.0075f);
    ...
    public void adjustCamera(Flashlight flashlight){
        flashlight.setCoord(Candys3Deluxe.inputManager.getX() + x, Candys3Deluxe.inputManager.getY() + y);
        Candys3Deluxe.cameraManager.move(getX(), y);
    }

    private float lookMechanic(float position, float mouse, float offset, int multiplier, int bounds){
        float distance = 0;
        float limit = Time.convertValue(bounds * 0.75f * multiplier);
        if (mouse < offset * 1){
