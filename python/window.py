...
jar_load = False
option = ""
options = {"Candys's 2 Deluxe": "candys2",
...
class RunJar(QThread):
    def run(self):
        ...
        if jar_load:
            print(option)
            python.execute.__execute__(option)
...
class App(QWidget):
    def __init__(self):
        super().__init__()
        self.windowTitle = "Candy's 3 Deluxe Launcher"
        self.windowIcon = "images/logo.png"
        ...
        id = QFontDatabase.addApplicationFont("res/fonts/candysFont.ttf")
        self.candysFont = QFontDatabase.applicationFontFamilies(id)
        id = QFontDatabase.addApplicationFont("res/fonts/captionFont.ttf")
    ...
    def __widgets__(self):
        ...
        self.image = QPixmap("res/images/deluxe_image.png")
        self.img_label = QLabel(self)
        self.img_label.setPixmap(self.image)
        self.img_label.setGeometry(0, 60, 1280, 480)

        if (os.path.getsize("res/user/account.txt") != 0):
            ...
            if (self.__authentication__(username, token)):
                ...
                with open("res/user/account.txt", "w") as file:
                    pass
    ...
    def __messagewindow__(self, text, icon, dialogFlag):
        ...
        if (icon == QMessageBox.Icon.Critical):
            ...
            self.dialog.setWindowTitle("Popup Message")
    ...
    def __authentication__(self, username, token):
        ...
        if (GAMEJOLT.__connection__()):
            if (GAMEJOLT.__authenticate__(username, token)):
                ...
                if (not user_id):
                    self.__messagewindow__("Something went wrong! Can't read user information!",
                                           QMessageBox.Icon.Critical, "ok")
                    return False
    ...
    def __login__(self, event):
        self.__authentication__(self.username_entry.text(), self.token_entry.text())
    ...
    def __load__(self, event):
        ...
        global option
        global GAMEJOLT
        if (not GAMEJOLT.__connection__()):
            self.__messagewindow__("No internet connection detected!", QMessageBox.Icon.Critical, "ok")
            return
        terminate = True
        jar_load = True
